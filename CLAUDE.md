...
Execution: `TradingHostedService` in the Worker runs a 5-second loop with up to 20 strategies in parallel, dispatching each active strategy to its matching handler — **except `FuturesArbitrage`, which is filtered out of that query and runs in `ArbitrageFastLoopService` on a 1-second tick** (its quotes come from the websocket cache, so a faster tick costs the exchanges nothing; a bot whose previous tick is still running is skipped, never queued). Ladder pacing lives in `ArbitrageHandler.MinSecondsBetweenOpens`, in seconds, so the loop interval can change without changing how fast levels fill. `FundingTickerRotationService` fires at :50 past each hour to (re)assign tickers to `HuntingFunding` bots based on upcoming funding rates, workspace-level uniqueness, and the symbol blacklist.
